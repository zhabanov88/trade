if (window._llmSettingsLoaded) {} else { window._llmSettingsLoaded = true; (function () {
    ...
    function renderConfigForm(providerId) {
        ...
        const savedKey = cur?.apiKey && cur.apiKey !== '***saved***' ? cur.apiKey : '';
        const savedKeyPlaceholder = cur?.apiKey ? '***сохранён***' : 'Вставьте API ключ...';
        ...
        <div class="llm-form">
            ...
            <div class="llm-field">
                ...
                <input class="sb-inp llm-inp" id="llm-apikey" type="password"
                    placeholder="${savedKeyPlaceholder}"
                    value="${savedKey}">
    ...
    function collectFormConfig(body) {
        const typedKey = body.querySelector('#llm-apikey')?.value || '';
        // Если поле пустое — сохраняем старый ключ из текущего конфига
        const apiKey = typedKey || (LS.current?.provider === LS.selected ? LS.current?.apiKey || '' : '');
        return {
