...
if (window._neuralUILoaded) {} else { window._neuralUILoaded = true; (function () {
    ...
    const NS = {
        ...
        loading: false,
        status: null,
        llmStatus: null,   // активный LLM провайдер из /api/llm/status
        analysis: null,
        trainResult: null,
        prediction: null,
        strategies: [],
        ...
        chatLoading: false,
        cfg: loadCfg(),
        // доступные инструменты из сервера
        availableInstruments: [],  // [{symbol, name, clickhouse_ticker}]
        availableIntervals: [],    // [{code, name, clickhouse_table, seconds}]
    };
    ...
    function getTable() {
        // Приоритет: выбранная таблица → активная таблица графика → fallback
        return NS.cfg.table ||
               window.app?._currentTable ||
               'market_data_minute';
    }

    function getIntervalCode() {
        return NS.cfg.intervalCode ||
               window.app?._currentResolution ||
               '1';
    }

    // Найти интервал по tradingview_code (для синка с графиком)
    ...
    function renderDashboard() {
        ...
        <div class="ns-page">
            <div class="ns-page-hdr">
                ...
                <button class="ns-btn ns-btn-primary" id="ns-analyze-btn">
    ...
    function bindSidebarEvents() {
        ...
        // Ticker input — автосохранение при изменении
    ...
    async function loadStatus() {
        try {
            ...
            if (!NS.cfg.intervalCode && window.app?._currentResolution) {
                ...
                else    { NS.cfg.table = window.app?._currentTable || 'market_data_minute'; }
            }
    ...
    async function loadTickers() {
        try {
            ...
            // Если тикер не задан — взять из активного графика или первый из списка
            if (!NS.cfg.ticker) {
                NS.cfg.ticker = getSymbolFromCHTicker(window.app?._currentTicker)
    ...
    async function loadIntervals() {
        try {
            ...
            // Синхронизировать выбранный интервал с графиком если ещё не выбран
    ...
    async function runTrain() {
        ...
        // Update intervalCode to match selected table
    ...
    function runCodeInChart(code) {
        try {
            ...
            // Merge setups
            Object.assign(window.app.setups||{}, window.app?.setups||{});
    ...
    function gatherAppContext() {
        ...
        if (bt) {
            ctx.backtest = {
                ticker:   bt.ticker || (bt.meta && bt.meta.ticker),
                table:    bt.table  || (bt.meta && bt.meta.table),
                fromDate: bt.fromDate || (bt.meta && bt.meta.fromDate),
                toDate:   bt.toDate   || (bt.meta && bt.meta.toDate),
                stats:    bt.stats,
                trades:   bt.trades ? bt.trades.length : 0,
                lastTrades: (bt.trades||[]).slice(-10).map(function(t){
                    return {
                        entry:  t.entry,
                        exit:   t.exit,
        ...
        if (NS.llmStatus) {
            ctx.llm = {
                provider: NS.llmStatus.provider,
                model:    NS.llmStatus.model,
            };
        }
    ...
    // tableOptions removed — using /api/intervals dynamic data
    ...
    function toast(msg) {
        ...
        el.style.cssText='position:fixed;bottom:60px;right:20px;background:#4caf50;color:#fff;padding:8px 16px;border-radius:6px;font-size:12px;font-family:monospace;z-index:99999;box-shadow:0 4px 20px rgba(0,0,0,.5)';
        el.textContent = msg;
    ...
    console.log('[NeuralUI] v1.0 loaded');
