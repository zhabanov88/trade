...
class LayoutManager {
    ...
    _subscribeToChanges() {
        ...
        setInterval(() => this._autoSaveToDB(), 120000);
    }

    async _autoSaveToDB() {
        if (this._dbSaveInProgress) return;
        ...
        try {
            const chartData = await this.getCurrentLayoutData();
            if (!chartData) return;
            ...
            const extendedData = {
                ...(typeof chartData === 'object' && chartData !== null ? chartData : {}),
                _appState: appState,
            };

            await apiClient.updateLayout(session.layoutId, {
                layout_data: extendedData,
                symbol:      session.symbol,
                interval:    session.interval,
            });

            this._lastDbSave = Date.now();
            console.log(`[LayoutMgr] Auto-saved to DB: layout ${session.layoutId}`);
        } catch (e) {
            console.warn('[LayoutMgr] DB auto-save failed:', e);
        } finally {
            this._dbSaveInProgress = false;
        }
    }
