...
class TradingApp {
    ...
    // Must be called AFTER auth, BEFORE initTradingView()
    async preloadLayoutsFromDB() {
        try {
            const existingSession = localStorage.getItem('tv_session');
            if (existingSession) {
                try {
                    const parsed = JSON.parse(existingSession);
                    if (parsed && parsed.layoutId) {
                        console.log('[Preload] tv_session exists with layoutId, skipping DB preload');
                        return;
                    }
                } catch (_) {}
            }

            console.log('[Preload] No saved session found, loading layouts from DB...');
            const layouts = await apiClient.getLayouts();

            if (!layouts || layouts.length === 0) {
                console.log('[Preload] No layouts in DB for this user');
                return;
            }

            console.log(`[Preload] Found ${layouts.length} layout(s) in DB`);

            const defaultLayout = layouts[0];

            let layoutDetail;
            try {
                layoutDetail = await apiClient.getLayout(defaultLayout.id);
            } catch (e) {
                console.warn('[Preload] Failed to fetch layout detail:', e);
                layoutDetail = defaultLayout;
            }

            const session = {
                layoutId:   layoutDetail.id,
                layoutName: layoutDetail.name,
                symbol:     layoutDetail.symbol || 'EUR',
                interval:   layoutDetail.interval || '1',
                savedAt:    Date.now()
            };
            localStorage.setItem('tv_session', JSON.stringify(session));
            console.log('[Preload] tv_session set:', session);

            const recentLayouts = layouts.slice(0, 5).map(l => ({
                id:       l.id,
                name:     l.name,
                symbol:   l.symbol || '',
                interval: l.interval || '',
                usedAt:   new Date(l.updated_at || l.created_at).getTime()
            }));
            localStorage.setItem('tv_recent_layouts', JSON.stringify(recentLayouts));
            console.log(`[Preload] tv_recent_layouts set: ${recentLayouts.length} items`);

        } catch (error) {
            console.error('[Preload] Failed to preload layouts from DB:', error);
        }
    }
