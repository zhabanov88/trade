...
(function () {
  ...
  async function loadScriptsData() {
    ...
    try {
      ...
      SP.indicators = all
      .filter(s => s.type_code === 'indicator' || s.type_id === 2)
      .map(ind => {
        const m = ind.meta
          ? (typeof ind.meta === 'string' ? JSON.parse(ind.meta) : ind.meta)
          : {};
        return { ...ind, outputs_schema: m.outputs_schema || ind.outputs_schema || [] };
      });
  ...
  async function cloneSetup(id) {
    try {
      const orig = SP.setups.find(s => s.id === id);
      if (!orig) return;
      const meta = JSON.parse(JSON.stringify(orig._meta || {}));
      const payload = {
        display_name: 'Копия ' + orig.display_name,
        system_name:  'setup_clone_' + Date.now(),
        type_id:      orig.type_id || 5,
        code:         orig.code || '{}',
        description:  orig.description || '',
        is_public:    orig.is_public ?? true,
        is_overlay:   orig.is_overlay ?? false,
        inputs_schema: orig.inputs_schema || [],
        meta,
      };
      ...
      await loadScriptsData();
    } catch(e) { alert('Ошибка клонирования: ' + e.message); }
  }
  ...
  function renderPage() {
    ...
    root.querySelectorAll('.sp-card-clone').forEach(btn => { btn.addEventListener('click', () => cloneSetup(+btn.dataset.id)); });
  }
