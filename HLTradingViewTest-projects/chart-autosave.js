/**
 * chart-autosave.js  v3
 *
 * 1. Автосохранение ОТКЛЮЧЕНО
 ...
 * 4. Save As создаёт новый layout
 *
...
(function () {
    ...
    async function _save(reason) {
        if (_isSaving || !_widget || _isLoading) return;
    ...
    function _patchAdapter(widget) {
        ...
        // saveChart — нажата кнопка Save или Save As
        ...
        adapter.saveChart = async function (chartData) {
            ...
            // chartData.id === undefined → "Save As" (новый лэйаут)
            // chartData.id !== undefined → "Save" (обновление существующего)
            const isSaveAs = !chartData.id;

            // "Save" — обновляем текущий layout
            if (currentId && !isSaveAs) {
                try {
            ...
            // "Save As" или нет активного layout → создаём новый
        ...
        adapter.getChartContent = async function (id) {
            _isLoading = true; if (_debTimer) { clearTimeout(_debTimer); _debTimer = null; }
    ...
    function _subscribe(widget) {
        ...
        widget.onChartReady(() => {
            ...
            console.log('[Autosave] v3 Ready (autosave OFF) | Active layout:', _getActiveLayoutName() || 'none');
        });
    }
    ...
    console.log('[Autosave] chart-autosave.js v3 loaded (autosave OFF)');
})();