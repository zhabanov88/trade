class IntervalSelector {
    constructor() {
        ...
        this._settingsCache = { light: null, dark: null };
    ...
    async init(widget) {
        ...
        // Теперь код остановится и дождется ответа от базы данных
        await this._loadSettings();

        this._buildDOM();
        this._injectCSS();

        // Теперь здесь this._displayFields уже будет заполнен данными из БД
            ...
            mutations.forEach((mutation) => {
                if (mutation.attributeName === 'class') {
                    ...
                    themeTimer = setTimeout(() => {
                        ...
                        console.log('[ISP] Смена темы:', theme);

                        // БЕЗ запроса в БД — берём из кэша
                        this._applyThemeFields();
                    }, 100);
                }
            });
        });
    ...
    async _loadSettings() {
        try {
            // Загружаем ОБЕ темы за один запрос
            const response = await fetch('/api/user/panel-settings/all', {
                credentials: 'include'
            });
            ...
            // Кэшируем обе темы
            this._settingsCache.light = data.light?.fields
                ? data.light.fields
                : JSON.parse(JSON.stringify(this._defaultFields));

            this._settingsCache.dark = data.dark?.fields
                ? data.dark.fields
                : JSON.parse(JSON.stringify(this._defaultFields));

        } catch (e) {
            console.error('[ISP] Ошибка загрузки из БД:', e);
            this._settingsCache.light = JSON.parse(JSON.stringify(this._defaultFields));
            this._settingsCache.dark = JSON.parse(JSON.stringify(this._defaultFields));
        }

        // Применяем текущую тему из кэша
        this._applyThemeFields();
    }

    // Новый метод — применить поля текущей темы из кэша
    _applyThemeFields() {
        const theme = document.body.classList.contains('light-theme') ? 'light' : 'dark';
        this._displayFields = this._settingsCache[theme]
            ?? JSON.parse(JSON.stringify(this._defaultFields));

        if (document.querySelector('.isp-fields-bar') || document.getElementById('isp-info')) {
            this._buildFieldsBar();
        }

        // Принудительно перекрасить уже видимые значения
        this._reapplyFieldColors();
    }

    // Новый метод — перекрашивает значения без перестройки DOM
    _reapplyFieldColors() {
        this._displayFields.forEach(f => {
            ...
            if (f.color) {
                ...
                el.style.color = ''; // сбросить на CSS-дефолт
            }
        });
    }
    ...
    async _saveSettings() {
        ...
        // Обновляем кэш текущей темы
        this._settingsCache[theme] = JSON.parse(JSON.stringify(this._displayFields));

        try {
            const payload = {
                theme: theme,
                fields_config: { fields: this._displayFields }
            };

            await fetch('/api/user/panel-settings', {
                method: 'POST',
            ...
            console.log('[ISP] Settings saved, theme:', theme);
        } catch (e) {
    ...
    async _loadIntervals() {
        try {
            ...
            list.filter(i => i.is_active).forEach(i => {
                ...
                else if (['1', '2', '3', '5', '10', '15', '30'].includes(c)) this.intervals.minutes.push(c);
