...
(function () {
    ...
    const AUTOSAVE_COOLDOWN_MS = 10000;

    function _autoSaveToDb(reason) {
        if (_autoSaving || !_widget) return;
        var layoutId = getActiveId();
        if (!layoutId) return;

        var now = Date.now();
        if (now - _lastAutoSaveAt < AUTOSAVE_COOLDOWN_MS && reason !== 'manual') {
            if (!_autoSaveDebounce) {
                var remaining = AUTOSAVE_COOLDOWN_MS - (now - _lastAutoSaveAt);
                _autoSaveDebounce = setTimeout(function () {
                    _autoSaveDebounce = null;
                    _autoSaveToDb(reason);
                }, remaining);
            }
            return;
        }
        ...
        try {
            _widget.save(function (layoutData) {
                if (!layoutData) { _autoSaving = false; return; }
                // Embed _appState so autosave preserves app state in DB
                if (typeof window._collectAppState === 'function') {
                    try { layoutData._appState = window._collectAppState(); } catch (_) {}
                }
                var chart = _widget.activeChart();
                var sym = chart.symbol();
                var iv = chart.resolution();
                apiClient.updateLayout(parseInt(layoutId), {
                    layout_data: layoutData,
                    symbol: sym,
                    interval: iv
                }).then(function () {
                }).catch(function (e) {
                    console.warn('[chart-session] Autosave failed:', e.message);
                }).finally(function () {
                    _autoSaving = false;
                });
            });
        } catch (e) {
            console.error('[chart-session] Autosave error:', e);
            _autoSaving = false;
        }
    }

    function _scheduleAutoSave(reason) {
    ...
    let _chartsCacheTs = 0;
    var CHARTS_CACHE_TTL = 30000; // 30s

    // Synchronous re-injection — runs in same frame as removal (no blink)
    function _reinjectRecentSync() {
        var doc = _getTVDoc() || document;
        var anchor = doc.querySelector('[data-name="save-load-menu-item-load"]');
        if (!anchor) return;
        var recent = readRecent();
        if (recent.length === 0) return;
        if (_chartsCache) {
            var validIds = {};
            _chartsCache.forEach(function (c) { validIds[String(c.id)] = true; });
            var filtered = recent.filter(function (r) { return validIds[String(r.id)]; });
            if (filtered.length > 0) _doInjectRecentItems(anchor, filtered);
        } else {
            _doInjectRecentItems(anchor, recent);
        }
        _recentCooldown = Date.now() + 300;
        window.DebugOverlay && DebugOverlay.info("[RL] sync re-inject done");
    }

    function _injectRecentItems() {
        if (_recentInjecting) { window.DebugOverlay && DebugOverlay.warn("[RL] blocked: _recentInjecting"); return; }
        if (Date.now() < _recentCooldown) { window.DebugOverlay && DebugOverlay.warn("[RL] blocked: cooldown, ms left:", _recentCooldown - Date.now()); return; }
        ...
        if (existing && existing.length > 0) {
            window.DebugOverlay && DebugOverlay.info("[RL] skip: " + existing.length + " items already in DOM");
            return;
        }

        window.DebugOverlay && DebugOverlay.warn("[RL] INJECTING, recent:", recent.length);
        _recentInjecting = true;
        var _useCache = (_chartsCache && Date.now() - _chartsCacheTs < CHARTS_CACHE_TTL);
        ...
        _chartsPromise.then(function (charts) {
            ...
            window.DebugOverlay && DebugOverlay.info("[RL] injecting " + filtered.length + " items (IDs: " + filtered.map(function(r){return r.id}).join(",") + ")");
            _doInjectRecentItems(anchor, filtered);
            // Monitor if TV removes our items
            setTimeout(function() {
                var doc2 = _getTVDoc() || document;
                var remaining = doc2.querySelectorAll('[data-rl-injected]');
                window.DebugOverlay && DebugOverlay.info("[RL] check@500ms: " + remaining.length + " items still in DOM");
            }, 500);
            setTimeout(function() {
                var doc3 = _getTVDoc() || document;
    ...
    function _debouncedInjectRecent() {
        if (_recentDebounceTimer) return;
        var _doc = _getTVDoc() || document;
        // Skip if menu not open
        if (!_doc.querySelector('[data-name="save-load-menu-item-load"]')) return;
        // Skip if our items already present
        if (_doc.querySelectorAll('[data-rl-injected]').length > 0) return;
        _recentDebounceTimer = setTimeout(function () {
            _recentDebounceTimer = null;
            _injectRecentItems();
        }, 30);
    }
            ...
            if (typeof w.loadChartFromServer === 'function') {
                ...
                setTimeout(function () {
                    ...
                    try {
                        var c = w.activeChart();
                        var iv = c.resolution();
                        var sessionIv = (readSession() || {}).interval;
                        if (iv && iv === sessionIv && window.intervalSelector) {
                            window.intervalSelector.currentInterval = iv;
    ...
    const adapter = {
            ...
            try {
                ...
                if (_layoutName) {
                    setTimeout(function () {
                        try {
                            var doc = _getTVDoc() || document;
                            var activeTitle = doc.querySelector('[data-active="true"] [data-name="list-item-title"]');
                            if (activeTitle && !activeTitle.textContent.trim()) {
                                activeTitle.textContent = _layoutName;
                            }
                        } catch (_) {}
                    }, 300);
                }

                // Sync interval selector buttons immediately
                // (onIntervalChanged is blocked by _isLoading, so we must sync manually)
                if (layout.interval && window.intervalSelector) {
                    try {
                        window.intervalSelector.currentInterval = layout.interval;
                        window.intervalSelector.updateActiveButton();
                    } catch (_) {}
                }
        ...
        removeStudyTemplate: function (studyTemplateInfo) {
            return fetch('/api/study-templates/' + encodeURIComponent(studyTemplateInfo.name), {
                method: 'DELETE', credentials: 'include'
            }).then(function () {});
        },
        saveStudyTemplate: function (studyTemplateData) {
            return fetch('/api/study-templates', {
                method: 'POST', credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ name: studyTemplateData.name, content: studyTemplateData.content })
            }).then(function () {});
        },
        ...
        getChartTemplateContent: function (templateName) {
            return fetch('/api/chart-templates/' + encodeURIComponent(templateName), {
                credentials: 'include'
            }).then(function (r) { return r.json(); })
              .catch(function () { return {}; });
        },
        saveChartTemplate: function (templateName, theme) {
            return fetch('/api/chart-templates', {
                method: 'POST', credentials: 'include',
                headers: { 'Content-Type': 'application/json' },
        ...
        loadDrawingTemplate: function (toolName, templateName) {
            return fetch('/api/drawing-templates/' + encodeURIComponent(toolName) + '/' + encodeURIComponent(templateName), {
                credentials: 'include'
            }).then(function (r) { return r.json(); })
              .then(function (data) { return data.content || ''; })
              .catch(function () { return ''; });
    ...
    function subscribe(widget) {
        ...
        widget.onChartReady(function () {
            ...
            // Pre-warm charts cache so first menu open is instant
            adapter.getAllCharts().then(function (charts) {
                _chartsCache = charts; _chartsCacheTs = Date.now();
            }).catch(function () {});
            ...
            var _recentObsFn = function (mutations) {
                // Check if TV removed our injected items
                for (var mi = 0; mi < mutations.length; mi++) {
                    var removed = mutations[mi].removedNodes;
                    for (var ri = 0; ri < removed.length; ri++) {
                        var nd = removed[ri];
                        if (nd.nodeType === 1) {
                            if ((nd.hasAttribute && nd.hasAttribute('data-rl-injected')) ||
                                (nd.querySelector && nd.querySelector('[data-rl-injected]'))) {
                                _reinjectRecentSync();
                                return;
                            }
                        }
