...
CREATE UNLOGGED TABLE IF NOT EXISTS forex_quotes_stage (
  ticker TEXT,
  ask_exchange INT,
  ask_price NUMERIC,
  bid_exchange INT,
  bid_price NUMERIC,
  participant_timestamp_ns BIGINT
);
"
...
" >/dev/null || true
  ...
  if [[ "$HEADER" != "ticker,ask_exchange,ask_price,bid_exchange,bid_price,participant_timestamp" ]]; then
...
TRUNCATE forex_quotes_stage;

\\copy forex_quotes_stage (ticker,ask_exchange,ask_price,bid_exchange,bid_price,participant_timestamp_ns) FROM ${TMPCSV_PSQL} WITH (FORMAT csv, HEADER true, DELIMITER ',', QUOTE '"');

INSERT INTO forex_quotes (ticker,ask_exchange,ask_price,bid_exchange,bid_price,participant_timestamp)
SELECT
  ticker,
  ask_exchange,
  ask_price,
  bid_exchange,
  bid_price,
  to_timestamp(participant_timestamp_ns / 1000000000.0)
FROM forex_quotes_stage;

UPDATE ingest_files
SET status='LOADED',
    loaded_at=now(),
    last_error=NULL,
    rows_loaded=(SELECT count(*) FROM forex_quotes_stage)
WHERE source_gz_path=$SOURCE_PATH_LIT
  AND extracted_csv_name=$CSV_NAME_LIT;
