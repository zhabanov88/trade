...
DROP VIEW IF EXISTS default.market_data_4hour;

-- Создаем материализованное представление с использованием временной зоны America/New_York
CREATE MATERIALIZED VIEW default.market_data_4hour
ENGINE = MergeTree()
ORDER BY (ticker, window_start)
AS
SELECT
    ticker,
    toStartOfInterval(toTimeZone(window_start, 'America/New_York'), INTERVAL 4 hour) AS window_start,
...
INSERT INTO default.market_data_4hour
SELECT
    ticker,
    toStartOfInterval(toTimeZone(window_start, 'America/New_York'), INTERVAL 4 hour) AS window_start,
    groupArray((open, window_start))[1].1 AS open, -- Первое значение Open
